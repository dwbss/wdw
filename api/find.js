...
${isFast ? '' : `PRIME DIRECTIVE — TIME-LIMITED FIRST: the single most valuable result is one that exists ONLY ${dayWord} or this weekend — a festival, fair, show, or one-off event. Your FIRST search must be "${safeLoc} what's on this weekend" or "${safeLoc} festival fair ${dayWord}". If ANY time-limited family event within range is verifiably on ${dayWord}, it MUST appear in the results, ranked FIRST, with "once": true. FLAG RULE: anything described as today-only, launches-today, one-day, this-weekend-only, or a short seasonal run MUST carry "once": true. If an option is both scarce AND obscure, "once" outranks "gem". Permanent venues fill the remaining slots. (If genuinely nothing time-limited is on, five great venues is the correct answer — never invent scarcity.)`}
${geoLine}
${sweepLine}
